...
static bool occupied_or_blocked(const Board *b, int x, int y, int self_id) {
    ...
    for (int j = 0; j < b->num_players; j++) {
        ...
        if (!b->blocked[j] && b->head_x[j] == (unsigned short)x && b->head_y[j] == (unsigned short)y) {
            return true;
        }
    }
    return false;
...
bool board_is_legal(const Board *b, int player_id, CenteMove m) {
    if (!board_is_inside(b, m.x, m.y)) return false;
...
int board_legal_moves(const Board *b, int player_id, CenteMove out[], int max_out) {
    ...
    for (int d = 0; d < 8; d++) {
        int nx = x + dx[d];
        int ny = y + dy[d];
        if (board_is_inside(b, nx, ny) && !occupied_or_blocked(b, nx, ny, player_id)) {
            if (k < max_out) {
                out[k].x = nx; out[k].y = ny;
            }
            k++;
        }
    }
...
void board_apply_move(Board *b, int player_id, CenteMove m) {
    ...
    if (val > 0) b->score[player_id] += (unsigned int)val;
