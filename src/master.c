...
int create_game_shared_memory(int width, int height, int num_players) {
    ...
    // Mapear memoria
    game_state = mmap(NULL, game_size, PROT_READ | PROT_WRITE, MAP_SHARED, game_shm_fd, 0);
...
void initialize_board() {
    ...
    for (int i = 0; i < (int)game_state->cantPlayers; i++) {
        int pos = game_state->players[i].qy * game_state->width + game_state->players[i].qx;
...
void initialize_players() {
    char player_names[][16] = {"Player1", "Player2", "Player3", "Player4", "Player5", "Player6", "Player7", "Player8", "Player9"};
...
int create_player_pipes() {
    for (int i = 0; i < (int)game_state->cantPlayers; i++) {
        ...
        if (flags != -1) fcntl(player_pipes[i][0], F_SETFL, flags | O_NONBLOCK);
    }
    return 0;
}
...
static int is_inside(int x, int y) {
    return x >= 0 && x < (int)game_state->width && y >= 0 && y < (int)game_state->high;
}
...
static int is_occupied(int x, int y, int self_id) {
    ...
    for (int j = 0; j < (int)game_state->cantPlayers; j++) {
        ...
        if (game_state->players[j].qx == x && game_state->players[j].qy == y && !game_state->players[j].blocked)
            return 1;
    }
    return 0;
}
...
static void apply_player_move(int pid, int direction) {
    ...
    if (!is_inside(nx, ny) || is_occupied(nx, ny, pid)) {
        game_state->players[pid].invalidMove++;
        // No bloquear de inmediato: permitir que intente otro movimiento en el próximo turno
        // game_state->players[pid].blocked = 1;
        return;
    }
    ...
    // lastMove eliminado del estado compartido
    // Limpiar la celda destino para que no muestre puntaje (se verá la cabeza por encima)
...
pid_t create_player_process(int player_id, const char* player_executable, int pipe_fd) {
    ...
    if (player_pid == 0) {
        ...
        sprintf(pipe_fd_str, "%d", pipe_fd);
        
        // Cerrar el extremo de lectura del pipe en el hijo
        // close(player_pipes[player_id][0]);
        
        execl(player_executable, player_executable, player_id_str, pipe_fd_str, NULL);
        perror("execl player");
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 3) {
        fprintf(stderr, "Uso: %s <width> <height> [player_executable_1] [player_executable_2] ...\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (argc >= 4) {
        ...
        printf("Usando configuración de jugadores por defecto: ./player1 y ./player2\n");
        num_players = 2;
        ...
        if (pret > 0) {
            for (int i = 0; i < num_players; i++) {
                ...
                if (pfds[i].revents & POLLIN) {
                    int direction;
                    ssize_t r = read(player_pipes[i][0], &direction, sizeof(direction));
                    if (r == (ssize_t)sizeof(direction)) {
    ...
    for (int i = 0; i < num_players; i++) {
        printf("Jugador %c | Puntaje: %u | Válidos: %u | Inválidos: %u | Estado: %s\n",
               'A' + i,
