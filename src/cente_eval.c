...
void phase_detect(const Board *b, const InfluenceMap *inf,
                  cente_phase *phase_out, cente_weights *w_out) {
  (void)inf;
  // Density: ratio of occupied cells
  int bw = b->width, h = b->height;
  int n = bw * h;
  int occ = 0;
  for (int i = 0; i < n; i++)
    if (b->cells[i] <= 0)
  ...
  cente_phase ph = density < 0.25f
                       ? PHASE_OPENING
                       : (density < 0.70f ? PHASE_MIDGAME : PHASE_ENDGAME);
  *phase_out = ph;
  cente_weights w;
  if (ph == PHASE_OPENING) {
    w.alpha = 0.25f;
...
static float claim_urgency(const Board *b, const InfluenceMap *inf,
                           CenteMove m) {
...
float tempo_score_1ply(const Board *b, int player_id, const InfluenceMap *inf,
                       CenteMove m) {
  (void)player_id;
  // Approx: IM ~ our local value - opponent local best
  float our =
      0.5f * centrality_score(b, m) + 0.5f * inf->t[m.y * b->width + m.x];
  ...
  for (int d = 0; d < 8; d++) {
    ...
    if (nx < 0 || ny < 0 || nx >= b->width || ny >= b->height)
      continue;
    float v = 0.5f * (1.0f - centrality_score(b, (CenteMove){nx, ny})) +
              0.5f * inf->t[ny * b->width + nx];
    if (v > opp_best)
      opp_best = v;
  }
...
float prior_cente(const Board *b, int player_id, const InfluenceMap *inf,
                  CenteMove m) {
  ...
  float prior = 0.20f * c + 0.25f * tval + 0.15f * tempo + 0.10f * stab +
                0.05f * (1.0f - risk) + 0.25f * claim;
  return clamp01(prior);
}

float value_eval(const Board *b, int player_id, const InfluenceMap *inf,
  ...
  for (int p = 0; p < b->num_players; p++)
    sum_scores += b->score[p];
  float score_ratio =
      sum_scores ? (float)b->score[player_id] / (float)sum_scores : 0.0f;
  t_head = clamp01(0.7f * t_head + 0.3f * score_ratio);
  float c = centrality_score(
      b, (CenteMove){b->head_x[player_id], b->head_y[player_id]});
  float mob = mobility_score(b, player_id);
  float stab = stability_proxy(
      b, inf, (CenteMove){b->head_x[player_id], b->head_y[player_id]});
  float risk = risk_proxy(
      b, inf, (CenteMove){b->head_x[player_id], b->head_y[player_id]});
  float tempo = 0.5f; // default neutral
  float V = w->alpha * t_head + w->beta * c + w->gamma * mob + w->delta * stab +
            w->eps * tempo - w->zeta * risk;
  // Clip to [0,1]
  return clamp01(V);
}
